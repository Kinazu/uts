<div class="w-full block mt-8">
    <div class="flex flex-wrap sm:flex-no-wrap justify-between">
        ...
        <!-- Log on to website for more projects -->
...
<div class="w-full block mt-8">
    ...
    ?>
    <div class="w-full sm:w ml-2 mb-6 bg-white rounded border-b-4 border-gray-300">
        @if ($students->count() > 0)
            <h3 class="text-gray-700 uppercase font-bold mb-2">Daftar Hasil Ujian</h3>
            <div class="flex items-center uppercase text-sm font-semibold bg-gray-600 text-white rounded-tl rounded-tr">
                <div class="w-1/12 text-left text-white py-2 px-4 font-semibold">No</div>
                <div class="w-3/12 text-left text-white py-2 px-4 font-semibold">Nama Siswa</div>
                <div class="w-2/12 text-left text-white py-2 px-4 font-semibold">Nilai Harian</div>
                <div class="w-2/12 text-left text-white py-2 px-4 font-semibold">Nilai UTS</div>
                <div class="w-2/12 text-left text-white py-2 px-4 font-semibold">Nilai UAS</div>
                <div class="w-2/12 text-left text-white py-2 px-4 font-semibold">Nilai Akhir</div>
            </div>
            @foreach ($students as $student)
                <div
                    class="flex items-center justify-between text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-1/12 text-left text-gray-600 py-2 px-4 font-medium ">{{ $loop->iteration }}</div>
                    <div class="w-3/12 text-left text-gray-600 py-2 px-4 font-medium ">{{ $student->nama }}</div>
                    <div class="w-2/12 text-left text-gray-600 py-2 px-4 font-medium ">{{ $student->harian }}
                    </div>
                    <div class="w-2/12 text-left text-gray-600 py-2 px-4 font-medium ">{{ $student->uts }}
                    </div>
                    <div class="w-2/12 text-left text-gray-600 py-2 px-4 font-medium ">{{ $student->uas }}
                    </div>
                    <div class="w-2/12 text-left text-gray-600 py-2 px-4 font-medium ">{{ $hasil }}
                    </div>
                </div>
            @endforeach
    </div>
